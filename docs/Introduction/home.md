...
Think of this guide as a paint brush. You use this paint brush to acquire your target language i.e paint your painting. But what kind of language resources (colors) you end up using will depend on your target language, so please refer to language specific language guides for concrete resources on what the most viable resources there are.

This guide will just provide a paint brush (method and mindset) so *you* can paint (learn your target language). This guide will not contain the colors (resources) you need for your painting (target language) because every painting (language) is different.
